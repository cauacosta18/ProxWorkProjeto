...
import { Getme, GetPessoa, UpdatePessoaAPI } from "../../services/ProxWorkAPI";
...
const AtualizacaoPessoa = () => {
    ...
    const handleSubmit = async (event: React.MouseEventHandler<HTMLButtonElement> | any) => {
            ...
            try{
                ...
                const response = await UpdatePessoaAPI(usuarioUpdateData, email);
                 
                console.log(response.data+ ""+response.status);
                
                const okPrestador = response?.status >= 200 && response?.status < 300;
                const okServico   = response?.status   >= 200 && response?.status   < 300;
...
export default AtualizacaoPessoa;